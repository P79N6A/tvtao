...
public class FocusedRelativeLayout extends RelativeLayout {
    ...
    public void setOutsideSroll(boolean scroll) {
        AppDebug.d("FocusedRelativeLayout", "setOutsideSroll scroll = " + scroll + ", this = " + this);
    ...
    private void initData() {
        ...
        // 设置焦点框图片
        this.setFocusResId(R.drawable.tradelink_focus);
        // 设置动画类型
        this.setFocusMode(FocusedRelativeLayout.FOCUS_ASYNC_DRAW);
        // 设置帧率
        this.setFrameRate(6);
        // 设置页面横向滚动后右边的padding
    ...
    private synchronized void init() {
        if ((hasFocus()) && (!(this.mOutsieScroll)) && (!(isInit()))) {
            ...
            AppDebug.d("FocusedRelativeLayout", "init mStartX = " + this.mStartX);
            setInit(true);
        }
    }
    ...
    public void dispatchDraw(Canvas canvas) {
        AppDebug.i("FocusedRelativeLayout", "dispatchDraw");
        super.dispatchDraw(canvas);
        drawFrame(canvas);
    }

    protected void onFocusChanged(boolean gainFocus, int direction, Rect previouslyFocusedRect) {
        AppDebug.d("FocusedRelativeLayout", "onFocusChanged this = " + this + ", mScreenWidth = " + this.mScreenWidth);
        synchronized (this) {
        ...
        if (!(gainFocus)) {
            ...
            AppDebug.i("FocusedRelativeLayout", "onFocusChanged: invalidate");
            invalidate();
        }
    }

    private void drawFrame(Canvas canvas) {
        AppDebug.i("FocusedRelativeLayout", "drawFrame: mCurrentFrame = " + this.mCurrentFrame + ", needDraw = " + this.mNeedDraw + ", child count = "
                + getChildCount() + ", this = " + this + ", has focus = " + hasFocus() + ", mOutsieScroll = " + this.mOutsieScroll + ", mIndex = "
                + this.mIndex + ", mFrameRate = " + this.mFrameRate + ", mode=" + this.mode);
    ...
    private void drawStaticFrame(Canvas canvas) {
        ...
        AppDebug.w("FocusedRelativeLayout", "drawFrame select view is null");
    }
    ...
    private void drawFirstFrame(Canvas canvas, boolean isScale, boolean isDynamic) {
        ...
        AppDebug.i("FocusedRelativeLayout", "drawFirstFrame: getImageRect");
        this.mFocusRect = getImageRect(scaleXValue, scaleYValue, dynamic);
        this.mCurrentrect = this.mFocusRect;
        AppDebug.d("FocusedRelativeLayout", "drawFirstFrame: mFocusRect = " + this.mFocusRect + ", this = " + this);

        drawScale(isScale);

        if (hasFocus()) {
            AppDebug.i("FocusedRelativeLayout", "drawFirstFrame: drawFocus");
            drawFocus(canvas, isDynamic);
        }
        this.mIsFirstFrame = false;
        this.mCurrentFrame += 1;
        AppDebug.i("FocusedRelativeLayout", "drawFirstFrame: invalidate");
        invalidate();
    }

    private void drawOtherFrame(Canvas canvas, boolean isScale, boolean isDynamic) {
        AppDebug.i("FocusedRelativeLayout", "drawOtherFrame, this = " + this);
    ...
    private void drawLastFrame(Canvas canvas, boolean isScale, boolean isDynamic) {
        AppDebug.d("FocusedRelativeLayout", "drawLastFrame, this = " + this);
    ...
    private void scaleSelectedView() {
        ...
        if (selectedView != null) {
            ...
            AppDebug.i("FocusedRelativeLayout", "scaleSelectedView: dstScaleX = " + dstScaleX + ", dstScaleY = " + dstScaleY + ", mScaleXValue = "
                    + this.mScaleXValue + ", mScaleYValue = " + this.mScaleYValue + ", Selected View = " + selectedView + ", this = " + this);
    ...
    private void scaleLastSelectedView() {
        if (this.mLastSelectedView != null) {
            ...
            float dstScaleX = 1.0F + diffX * currentFrame / frameRate;
            float dstScaleY = 1.0F + diffY * currentFrame / frameRate;
            AppDebug.i("FocusedRelativeLayout", "scaleLastSelectedView: dstScaleX = " + dstScaleX + ", dstScaleY = " + dstScaleY + ", mScaleXValue = "
                    + this.mScaleXValue + ", mScaleYValue = " + this.mScaleYValue + ", mLastSelectedView = " + this.mLastSelectedView + ", this = "
    ...
    private void drawScale(boolean isScale) {
        AppDebug.i("FocusedRelativeLayout", "drawScale: mCurrentFrame = " + this.mCurrentFrame + ", mScaleXValue = " + this.mScaleXValue
    ...
    private void drawFocus(Canvas canvas, boolean isDynamic) {
        AppDebug.i("FocusedRelativeLayout", "drawFocus: mCurrentFrame = " + this.mCurrentFrame + ", mScaleXValue = " + this.mScaleXValue
                + ", mScaleYValue = " + this.mScaleYValue + ", this = " + this);
    ...
    private void drawStaticFocus(Canvas canvas) {
        ...
        float dstScaleX = 1.0F + diffX * currentFrame / frameRate;
        float dstScaleY = 1.0F + diffY * currentFrame / frameRate;
        AppDebug.i("FocusedRelativeLayout", "drawStaticFocus: mCurrentFrame = " + this.mCurrentFrame + ", dstScaleX = " + dstScaleX + ", dstScaleY = "
                + dstScaleY + ", mScaleXValue = " + this.mScaleXValue + ", mScaleYValue = " + this.mScaleYValue + ", this = " + this);

        AppDebug.i("FocusedRelativeLayout", "drawStaticFocus: getImageRect");
        Rect dstRect = getImageRect(dstScaleX, dstScaleY, false);
        if (dstRect == null) {
            AppDebug.i("FocusedRelativeLayout", "drawStaticFocus: dstRect = null");
            return;
        }
    ...
    private void drawDynamicFocus(Canvas canvas) {
        ...
        AppDebug.i("FocusedRelativeLayout", "drawDynamicFocus: mCurrentFrame = " + this.mCurrentFrame + ", dstRect.left = " + dstRect.left
                + ", dstRect.right = " + dstRect.right + ", dstRect.top = " + dstRect.top + ", dstRect.bottom = " + dstRect.bottom + ", this = "
                + this);
    ...
    private Rect getImageRect(float scaleXValue, float scaleYValue, boolean isDynamic) {
        ...
        AppDebug.d("FocusedRelativeLayout", "getImageRect imgRect = " + imgRect);
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        int direction;
        synchronized (this) {
            if ((System.currentTimeMillis() - this.mKeyTime <= this.KEY_INTERVEL) || (getState() == 1)
                    || (System.currentTimeMillis() - this.mScrollTime < 100L)) {
                AppDebug.i("FocusedRelativeLayout", "onKeyDown mAnimationTime = " + this.mKeyTime + " -- current time = " + System.currentTimeMillis());
                return true;
            }
            this.mKeyTime = System.currentTimeMillis();
        }
        ...
        AppDebug.d("FocusedRelativeLayout", "onKeyDown v = " + v);
        if ((v != null)) {
    ...
    private void scroll() {
        ...
        AppDebug.d("FocusedRelativeLayout", "scroll left = " + location[0] + ", right = " + right);
        ...
        AppDebug.d("FocusedRelativeLayout", "scroll conrtainer left = " + this.mStartX);
    ...
    public void smoothScrollBy(int dx, int duration) {
        AppDebug.w("FocusedRelativeLayout", "smoothScrollBy dx = " + dx);
    ...
    private boolean checkFocusPosition() {
        ...
        AppDebug.i("FocusedRelativeLayout", "checkFocusPosition: getImageRect");
        Rect dstRect = getImageRect(this.mScaleXValue, this.mScaleYValue, false);

        return ((Math.abs(dstRect.left - this.mCurrentrect.left) <= 5) && (Math.abs(dstRect.right - this.mCurrentrect.right) <= 5)
                && (Math.abs(dstRect.top - this.mCurrentrect.top) <= 5) && (Math.abs(dstRect.bottom - this.mCurrentrect.bottom) <= 5));
    }

    public void computeScroll() {
        AppDebug.i("FocusedRelativeLayout", "computeScroll ");
        if (this.mScroller.computeScrollOffset()) {
            scrollTo(this.mScroller.getCurrX(), this.mScroller.getCurrY());
            AppDebug.i("FocusedRelativeLayout", "computeScroll mScroller.getCurrX() = " + this.mScroller.getCurrX());
        }
    ...
    class HotScroller extends Scroller {
        ...
        public boolean computeScrollOffset() {
            ...
            AppDebug.d("FocusedRelativeLayout", "computeScrollOffset isFinished = " + isFinished + ", mOutsieScroll = "
                    + FocusedRelativeLayout.this.mOutsieScroll + ", needInvalidate = " + needInvalidate + ", this = " + this);
