...
public class FlowLayoutManager extends LayoutManager {
    ...
    public void setLine(int lineNumberMax){
        this.lineNumberMax = lineNumberMax;
    ...
    private void fillLayout(Recycler recycler, State state) {
        if (!state.isPreLayout()) {
            ...
            int resultline = 0;
            if (lineNumberMax!=0&&lineNumberMax<this.lineRows.size()){
                    resultline = lineNumberMax;
            }else {
    ...
    private void formatAboveRow() {
        ...
        for(int i = 0; i < views.size(); ++i) {
            ...
            if ((float)((Rect)this.allItemFrames.get(position)).top < this.row.cuTop + (this.row.maxHeight - (float)((Item)views.get(i)).useHeight) / 2.0F) {
                Rect frame = (Rect)this.allItemFrames.get(position);
                if (frame == null) {
                    frame = new Rect();
                }

                frame.set(((Rect)this.allItemFrames.get(position)).left, (int)(this.row.cuTop + (this.row.maxHeight - (float)((Item)views.get(i)).useHeight) / 2.0F), ((Rect)this.allItemFrames.get(position)).right, (int)(this.row.cuTop + (this.row.maxHeight - (float)((Item)views.get(i)).useHeight) / 2.0F + (float)this.getDecoratedMeasuredHeight(view)));
