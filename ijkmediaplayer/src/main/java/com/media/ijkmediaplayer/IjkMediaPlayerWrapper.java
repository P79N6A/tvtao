...
public class IjkMediaPlayerWrapper implements IMediaPlayer {
    ...
    private static boolean hardDecode = true;
    ...
    private void init() {
        hardDecode = isHarCode();
        AppDebug.d(TAG, "IjkMediaPlayerWrapper init hardDecode : " + hardDecode);
        mPlayer = new IjkMediaPlayer();
        mPlayer.native_setLogLevel(IjkMediaPlayer.IJK_LOG_SILENT);
        mPlayer.setOnMediaCodecSelectListener(new IjkMediaPlayer.OnMediaCodecSelectListener() {
            @Override
            public String onMediaCodecSelect(IMediaPlayer iMediaPlayer, String s, int i, int i1) {
                String codec = IjkMediaPlayer.DefaultMediaCodecSelector.sInstance.onMediaCodecSelect(iMediaPlayer, s, i, i1);
                return codec;
            }
        });
        mPlayer.setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "mediacodec", hardDecode ? 1 : 0);
        mPlayer.setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "mediacodec-auto-rotate", hardDecode ? 1 : 0);
        mPlayer.setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "mediacodec-handle-resolution-change", hardDecode ? 1 : 0);
