...
public class NewCollectsAdapter extends LifeUiBaseAdapter {
    ...
    public NewCollectsAdapter(Context context, ArrayList<NewCollection> collects) {
        super(context);
    ...
    public void fillView(int position, View convertView, boolean newConvertView, ViewGroup parent) {
        ...
        NewCollection c = mCollects.get(position);
        if (c == null) {
            return;
        }
        ...
        if (mPauseWork) {
            ...
            // ImageLoaderManager.getImageLoaderManager(mContext).displayImage(c.getPicUrl(),
            // itemLayoutForCollect.getItemImageView(), options);
            ImageLoaderManager.getImageLoaderManager(mContext).displayImage(c.getItemPic(),
        ...
        // 设置标题
        itemLayoutForCollect.getItemSummaryView().setText(c.getItemTitle());
        // 价格
