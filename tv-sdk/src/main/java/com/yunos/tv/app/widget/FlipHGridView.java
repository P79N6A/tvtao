...
import com.yunos.tv.app.widget.ItemFlipHScroller.ItemFlipScrollerListener;
...
public class FlipHGridView extends HGridView {
	...
	private FlipRunnable mFlipRunnable;//动画算法的管理类  （动画从左到右）
	private List<Integer> mHeaderViewList;//headerView的列表（行列信息）
	private List<Integer> mFooterViewList;//footerView的列表（行列信息）
	...
	public FlipHGridView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init();
	}

	public FlipHGridView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	/**
	 * 取得指定index所在列的第一个item的还剩余的距离
	 * @param index
	 * @return
	 */
	public int getFlipRowFirstItemLeftMoveDistance(int index){
		return mFlipRunnable.getFlipRowFirstItemLeftMoveDistance(index);
	}
	
	protected void correctTooWide(int numLines, int horizontalSpacing, int childCount) {
		//锁住动画
		mLockFlipAnim = true;
		super.correctTooWide(numLines, horizontalSpacing, childCount);
		mLockFlipAnim = false;
	}
	
	protected void correctTooNarrow(int numLines, int verticalSpacing, int childCount) {
		//锁住动画
		mLockFlipAnim = true;
		super.correctTooNarrow(numLines, verticalSpacing, childCount);
		mLockFlipAnim = false;
	}
	...
	@Override
	protected void offsetChildrenLeftAndRight(final int offset) {
	...
	public boolean onKeyUp(int keyCode, KeyEvent event) {
		if(!mFlipRunnable.isFinished() && (keyCode == KeyEvent.KEYCODE_DPAD_RIGHT || keyCode == KeyEvent.KEYCODE_DPAD_LEFT)){
			//按键抬起时，如果动画还示结束，开始水平减速显示
	...
	protected void layoutChildren() {
		if(mFlipRunnable.isFinished() == false){
			...
			//为了保证在向右滚动的时候如果要重新布局的话lastPos必须不小于现在的lastPos,
			//防止在动画过程中重新布局后lastPos变小。因为原生的GridView是水平整行布局的
			if(mFlipRunnable.isRight()){
		...
		try {
			...
			case LAYOUT_FROM_MIDDLE:
				if (newSel != null) {
					...
					sel = fillSelectionMiddle(childrenLeft, childrenRight);
				}
				break;
			case LAYOUT_SET_SELECTION:
				if (newSel != null) {
	...
	/**
	 * 向右：取得计算新增列的参照item，现在为最后一列的第一个
	 * 向左：取第一列的最后一个
	 * （现在修改成新增列参考的位置为最后一列的第一行的item，
	 * 	  因为这个item是整列里面的参照物，而后面一列是以前面一列为参照物的）
	 */
	@Override
	protected int getFillGapNextChildIndex(boolean isRight){
		if(mTouchMode < 0){
			if(isRight){
				...
				int up = (last - mHeaderViewList.size() + 1)% getNumLines();
				if(up <= 0){
					up = getNumLines();
				}
				int childIndex = (last - (up - 1)) - first;
				...
				int first = getFirstVisiblePosition();
				//当屏幕内没有HeaderView的时候就取列表最前一列的最后一行
				if(first >= mHeaderViewList.size()){
					int numLines = getNumLines();
					int childIndex = numLines - 1;
			...
			return super.getFillGapNextChildIndex(isRight);
		}
	}
	...
	public void addHeaderView(View v) {
		if(v != null && v instanceof FlipGridViewHeaderOrFooterInterface){
			...
			int horCount = headView.getHorCount();
			int verCount = headView.getVerticalCount();
			mHeaderViewList.add(mFlipRunnable.makeHeaderAndFooterViewInfo(verCount, horCount));
	...
	public void addFooterView(View v){
		if(v != null && v instanceof FlipGridViewHeaderOrFooterInterface){
			...
			int horCount = headView.getHorCount();
			int verCount = headView.getVerticalCount();
			mFooterViewList.add(mFlipRunnable.makeHeaderAndFooterViewInfo(verCount, horCount));
	 ...
	 */
	 public void addFooterViewInfo(View v) {
			if(v != null && v instanceof FlipGridViewHeaderOrFooterInterface){
				...
				int horCount = footerView.getHorCount();
				int verCount = footerView.getVerticalCount();
				mFooterViewList.add(mFlipRunnable.makeHeaderAndFooterViewInfo(verCount, horCount));
			}
			else{
				throw new IllegalStateException("Cannot add FlipGridView footer view to list header view maybe not FlipGridViewHeaderOrFooterInterface");
	...
	protected boolean detachOffScreenChildren(boolean isRight) {
		...
		if (isRight) {
			final int left = 0;//getPaddingTop();
			...
			final int right = getWidth();// - getPaddingBottom();
	...
	/**
	 * 取得当前选中位置的偏移跟当前列的差值距离（现在没有用到这段代码，后续可以会使用
	 ...
	 * @param columnStart
	 * @param selectedColumnView
	 * @return
	 */
	protected int getSelectedColumnItemOffset(int columnStart, View selectedColumnView){
		int offset = 0;
		int tag = (Integer)selectedColumnView.getTag();
		int numLines = getNumLines();
		int tagColumnStart = getColumnStart(tag);
		int upColumnDelta = 0;
		if(tagColumnStart > (columnStart + numLines)){
			upColumnDelta = ((tagColumnStart - columnStart) / numLines) - 1;
			offset -= ((selectedColumnView.getWidth() + getHorizontalSpacing())) * upColumnDelta;
			if(columnStart < mHeaderViewInfos.size()){
				offset -= (selectedColumnView.getWidth() + getHorizontalSpacing());
			}
			int rowStartLeft = getFlipRowFirstItemLeftMoveDistance(columnStart);
			int tagRowLeft = getFlipRowFirstItemLeftMoveDistance(tagColumnStart);
			int tagLeft = getFlipItemLeftMoveDistance(tag, 0);
//			offset += tagLeft - tagRowLeft;
//			offset -= rowStartLeft - tagRowLeft;
			if(DEBUG){
				Log.i(TAG, "getSelectedRowItemOffset mReferenceViewInSelectedRow="+selectedColumnView.getTag()+
						" tagRowStart="+tagColumnStart+" rowStart="+columnStart+" offset="+offset+
						" getWidth="+selectedColumnView.getWidth()+" upRowDelta="+upColumnDelta+
	...
	private class FlipRunnable implements Runnable {
		private ItemFlipHScroller mItemFlipScroller;
		private SparseArray<Integer> mFlipItemRightList;
		private int mHeaderViewDelta = 0;
		private OnFlipRunnableListener mOnFlipRunnableListener;
		FlipRunnable() {
			mFlipItemRightList = new SparseArray<Integer>();
			mItemFlipScroller = new ItemFlipHScroller();
					...
					if(childView != null && delta != 0){
						if(childIndex < getHeaderViewsCount()){
							...
							if(childView != null && childView instanceof FlipGridViewHeaderOrFooterInterface){
								...
								int headerChildCount = headerView.getVerticalCount() * headerView.getHorCount();
								if(secondIndex < headerChildCount){
									if(secondIndex == (headerChildCount -1)){
										...
										mFlipItemRightList.put(getFlipItemMapKey(childIndex, -1), childView.getRight());
									}
									View headerChildView = headerView.getView(secondIndex);
									if(headerChildView != null){
										headerChildView.offsetLeftAndRight(delta - mHeaderViewDelta);
										mFlipItemRightList.put(getFlipItemMapKey(childIndex, secondIndex), headerChildView.getRight());
									}
								}
							}
							else{
								childView.offsetLeftAndRight(delta);
								mFlipItemRightList.put(getFlipItemMapKey(childIndex, 0), childView.getRight());
							}
						}
						else{
							//adapter
		...
		public boolean lockKeyEvent(int keyCode){
			...
			if(DEBUG){
				Log.i(TAG, "lockKeyEvent isFinished="+isFinished+" keyCode="+keyCode+" isDown="+mItemFlipScroller.isRight());
			}
			if(!isFinished){
				//在向右滚动的时候按左方向键无效
				if(keyCode == KeyEvent.KEYCODE_DPAD_LEFT && mItemFlipScroller.isRight()){
					return true;
				}
				//在向左滚动的时候按右方向键无效
				else if(keyCode == KeyEvent.KEYCODE_DPAD_RIGHT && !mItemFlipScroller.isRight()){
					return true;
				}
				//使上下按键无效
		...
		public boolean isRight(){
			return mItemFlipScroller.isRight();
		}
				...
				for (int i = childCount - 1; i >= 0; i--) {
					...
					if(index < getHeaderViewsCount()){
						...
						if(childView != null && childView instanceof FlipGridViewHeaderOrFooterInterface){
							FlipGridViewHeaderOrFooterInterface headerView = (FlipGridViewHeaderOrFooterInterface)childView;
							int headerChildCount = headerView.getVerticalCount() * headerView.getHorCount();
							//取得HeaderView上次的right
							Integer childRight = mFlipItemRightList.get(getFlipItemMapKey(index, -1));
							if(childRight != null){
								//先处理HeaderView
								int delta = childRight - childView.getRight();
								if(DEBUG){
									Log.i(TAG, "checkFlipFastScroll headerView childRight="+childRight+" headerView right="+childView.getBottom()+" delta="+delta);
								}
								if(delta != 0){
								...
								for(int headerChildIndex = headerChildCount - 1; headerChildIndex >= 0; headerChildIndex--){
									childRight = mFlipItemRightList.get(getFlipItemMapKey(index, headerChildIndex));
									if(childRight != null){
										View headerChildView = headerView.getView(headerChildIndex);
										if(headerChildView != null){
											delta = childRight - headerChildView.getRight();
											if(DEBUG){
												Log.i(TAG, "checkFlipFastScroll headerChild headerChildIndex="+headerChildIndex+
													" childRight="+childRight+" headerChildView right="+headerChildView.getRight());
											}
											if(delta != 0){
												mItemFlipScroller.setFastScrollOffset(index, headerChildIndex, delta);
												headerChildView.offsetLeftAndRight(delta);
											}
										}
									}
								}
							...
							Integer childRight = mFlipItemRightList.get(getFlipItemMapKey(index, 0));
							if(childRight != null){
								int delta = childRight - childView.getRight();
								if(delta != 0){
									mItemFlipScroller.setFastScrollOffset(index, 0, delta);
						...
						if(DEBUG){
							Log.i(TAG, "checkFlipFastScroll adapter index="+index+
									" childRight="+childRight+" currRight="+childView.getRight());
						}
						if(childRight != null){
							int delta = childRight - childView.getRight();
							if(delta != 0){
		...
		public void startScroll(int distance){
			if(mItemFlipScroller.isFinished()){
				...
				mItemFlipScroller.setRowCount(getNumLines());
		...
		public int getFlipRowFirstItemLeftMoveDistance(int index){
			if(mItemFlipScroller.isFinished() == false){
				return mItemFlipScroller.getFlipRowFirstItemLeftMoveDistance(index);
			}
			return 0;
		 ...
		 * @return int
		 */
		public int makeHeaderAndFooterViewInfo(int verCount, int horCount){
			return (verCount << 16) | horCount;
		}
	    ...
	    private void setEachFrame(boolean finishImmediately){
			...
			if(more){
						...
						if(view != null && view instanceof FlipGridViewHeaderOrFooterInterface){
							...
							for(int headerChildIndex = headerChildCount - 1; headerChildIndex >= 0; headerChildIndex--){
								...
								if(headerChildIndex >= (headerChildCount - 1)){
									...
									view.offsetLeftAndRight(headerViewDelta);
									//保存HeaderView的位置
									mFlipItemRightList.put(getFlipItemMapKey(index, -1), view.getRight());
								}
								View headerChildView = headerView.getView(headerChildIndex);
								if(headerChildView != null){
									headerChildView.offsetLeftAndRight(delta - headerViewDelta);
									mFlipItemRightList.put(getFlipItemMapKey(index, headerChildIndex), headerChildView.getRight());
								}
							}
						}
						else{
							...
							mFlipItemRightList.put(getFlipItemMapKey(index, 0), view.getRight());
						}
					}
					else{
						//adapter
						...
						if(delta != 0){
							if(DEBUG){
								if(index == 19 ||
										index == 13 ||
										index == 24){
									int before = view.getRight();
									view.offsetLeftAndRight(delta);
									Log.i(TAG, "run index="+index+" before="+before+
											" delta="+delta+" after="+view.getRight());
								}
								else{
									view.offsetLeftAndRight(delta);
								}
				...
				detachOffScreenChildren(mItemFlipScroller.isRight());
				fillGap(mItemFlipScroller.isRight());
				onScrollChanged(0, 0, 0, 0);
				invalidate();
				postOnAnimation(this);
				mItemFlipScroller.addGridView(getFirstVisiblePosition(), getLastVisiblePosition(), mItemFlipScroller.isRight());
				//取得当前新增的item位置
					...
					if(preFirst <= (headerViewCount - 1)){
						...
						for(int i = preFirst; i <= (headerViewCount - 1); i++){
							...
							if(index >= 0){
								...
								if(childView != null && childView instanceof FlipGridViewHeaderOrFooterInterface){
									FlipGridViewHeaderOrFooterInterface headerView = (FlipGridViewHeaderOrFooterInterface)childView;
									mFlipItemRightList.put(getFlipItemMapKey(i, -1), childView.getRight());
									int headerChildCount = headerView.getVerticalCount() * headerView.getHorCount();
									for(int headerChildIndex = headerChildCount - 1; headerChildIndex >= 0; headerChildIndex--){
										View headerChildView = headerView.getView(headerChildIndex);
										if(headerChildView != null){
											mFlipItemRightList.put(getFlipItemMapKey(i, headerChildIndex), headerChildView.getRight());
										}
									}
								}
								else{
									mFlipItemRightList.put(getFlipItemMapKey(i, 0), childView.getRight());
								}
							}
						}
					}
						...
						for(int i = (headerViewCount - 1) + 1; i < preFirst; i++){
							int index = i - currFirst;
							if(index >= 0){
								mFlipItemRightList.put(getFlipItemMapKey(i, 0), getChildAt(index).getRight());
							}
						}
					}
					else{
