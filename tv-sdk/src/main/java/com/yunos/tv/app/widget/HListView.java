...
public class HListView extends AbsHListView {
	...
	protected View moveSelection(View oldSel, View newSel, int delta, int childrenLeft, int childrenRight) {
		...
		if (delta > 0) {
			...
			sel = makeAndAddView(selectedPosition, oldSel.getRight() + dividerWidth + mSpacing, true, mListPadding.top, true);
			...
			// Fill in views above and below
			if (!mStackFromBottom) {
				fillLeft(mSelectedPosition - 2, sel.getLeft() - dividerWidth - mSpacing);
				adjustViewsLeftOrRight();
				fillRight(mSelectedPosition + 1, sel.getRight() + dividerWidth + mSpacing);
			} else {
				fillRight(mSelectedPosition + 1, sel.getRight() + dividerWidth + mSpacing);
				adjustViewsLeftOrRight();
				fillLeft(mSelectedPosition - 2, sel.getLeft() - dividerWidth - mSpacing);
	...
	private void fillLeftAndRight(View sel, int position) {
		final int dividerWidth = mDividerWidth;
		if (!mStackFromBottom) {
			fillLeft(position - 1, sel.getLeft() - dividerWidth - mSpacing);
			adjustViewsLeftOrRight();
			fillRight(position + 1, sel.getRight() + dividerWidth + mSpacing);
		} else {
			fillRight(position + 1, sel.getRight() + dividerWidth + mSpacing);
			adjustViewsLeftOrRight();
		...
		if (lastPosition == mItemCount - 1 && childCount > 0) {
			...
			if (rightOffset > 0 && (mFirstPosition > 0 || firstLeft < mListPadding.left)) {
				...
				if (mFirstPosition > 0) {
					...
					// more rows, if
					// possible
					fillLeft(mFirstPosition - 1, firstChild.getLeft() - mDividerWidth - mSpacing);
			...
			if (leftOffset > 0) {
				if (lastPosition < mItemCount - 1 || lastRight > end) {
					...
					if (lastPosition < mItemCount - 1) {
						...
						// with more rows, if
						// possible
						fillRight(lastPosition + 1, lastChild.getRight() + mDividerWidth + mSpacing);
						// Close up the remaining gap
						adjustViewsLeftOrRight();
					}
	...
	protected void superOnMeasure(int widthMeasureSpec, int heightMeasureSpec){
		// Sets up mListPadding
		super.onMeasure(widthMeasureSpec, heightMeasureSpec);
		...
		mItemCount = mAdapter == null ? 0 : mAdapter.getCount();
		if (mItemCount > 0 && (widthMode == MeasureSpec.UNSPECIFIED || heightMode == MeasureSpec.UNSPECIFIED)) {
			final View child = obtainView(mSelectedPosition, mIsScrap);


			measureScrapChild(child, mSelectedPosition, heightMeasureSpec);
	...
	private ArrowScrollFocusResult arrowScrollFocused(final int direction) {
		...
		if (selectedView != null && selectedView.hasFocus()) {
			...
			if (direction == FOCUS_RIGHT) {
				...
				final int xSearchPoint = (selectedView != null && selectedView.getRight() < listRight) ? selectedView.getRight()
						: listRight;
				mTempRect.set(xSearchPoint, 0, xSearchPoint, 0);
			}
