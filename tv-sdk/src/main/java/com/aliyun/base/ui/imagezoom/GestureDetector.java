...
//                Log.i(TAG, "======" + distance + "," + mTouchSlopSquare + ",=====deltaX=" + deltaX);
                if (distance > mTouchSlopSquare) {
                    handled = mListener.onScroll(mCurrentDownEvent, ev, scrollX, scrollY);
                    Log.i(TAG, "-------distance > mTouchSlopSquare---------------" + handled);
                    mLastMotionX = x;
                    mLastMotionY = y;
                	...
                	handled = mListener.onScroll(mCurrentDownEvent, ev, scrollX, scrollY);
                	Log.i(TAG, "-------distance > mTouchSlopSquare----else-----------" + handled);
                	mLastMotionX = x;
                    mLastMotionY = y;
                    mAlwaysInTapRegion = false;
//                	mHandler.removeMessages(TAP);
//                    mHandler.removeMessages(SHOW_PRESS);
//                    mHandler.removeMessages(LONG_PRESS);// by leiming 2013.5.14 22:08
                }
                ...
                handled = mListener.onScroll(mCurrentDownEvent, ev, scrollX, scrollY);
                Log.i(TAG, "-------Math.abs(scrollX) >= 1---------------" + handled);
                mLastMotionX = x;
                mLastMotionY = y;
            } else {
            	handled = mListener.onScroll(mCurrentDownEvent, ev, scrollX, scrollY);
            	Log.i(TAG, "-------Math.abs(scrollX) >= 1---else------------" + handled);
                mLastMotionX = x;
                mLastMotionY = y;
            }
            break;
