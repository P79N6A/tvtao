...
//	protected int mImageWidth = -1;// 图片的原始宽度
//	protected int mImageHeight = -1;// 图片的原始高度
	protected float mFitScreenScale;
//	protected boolean mIsDefaultImage = true;
//	
...
//			center(true, true);
//			if (mFitToScreen) {
//				Log.i("aabb", "--------------onLayout----------居中!---------------");
//				zoomTo(mFitScreenScale, MobileInfo.getScreenWidthDip()/2, MobileInfo.getScreenHeightPx()/2, 100);
//				invalidate();
//			} else {
//				center(true, true);
//			}
...
//		}
	...
	protected void jisuanZoom(Drawable drawable) {
		if ( drawable == null) {
			mFitScreenScale = mMaxZoom = mMinZoom = 1F; // 默认图
			return ;
		}
...
//		float max = Math.max( fw, fh ) * 4;
		float scaleWidth = MobileInfo.getScreenWidthPx(getContext()) / (float) drawable.getIntrinsicWidth();
		float scaleHeight = MobileInfo.getScreenHeightPx(getContext()) / (float) drawable.getIntrinsicHeight();
//		mFitScreenScale = Math.min(scaleWidth, scaleHeight);
		mFitScreenScale = scaleWidth;//最重要的就是宽度
		mMaxZoom = mFitScreenScale * 4.0f;
		mMinZoom = mFitScreenScale * 0.8f;
	}
	...
	protected void getProperBaseMatrix2( Drawable bitmap, Matrix matrix ) {
		float viewWidth = getWidth();
		float viewHeight = getHeight();
//		float viewWidth = MobileInfo.getScreenWidthPx();
//		float viewHeight = MobileInfo.getScreenHeightPx();
		float w = bitmap.getIntrinsicWidth();
		float h = bitmap.getIntrinsicHeight();
//		Log.i("aabb", "-------------getIntrinsicWidth----------------------" + w);
//		Log.i("aabb", "-------------getIntrinsicHeight----------------------" + h);
		matrix.reset();
		float widthScale = Math.min( viewWidth / w, mMaxZoom );
		float heightScale = Math.min( viewHeight / h, mMaxZoom );
		float scale = Math.min( widthScale, heightScale );
		mFitScreenScale = scale;
//		Log.i("aabb", "-------------mFitScreenScale----------------------" + mFitScreenScale);
		matrix.postScale( scale, scale );
		matrix.postTranslate( ( viewWidth - w * scale ) / mMaxZoom, ( viewHeight - h * scale ) / mMaxZoom );
	}
	...
	protected boolean panBy( double dx, double dy ) {
		...
		center( true, true );
		Log.i(TAG, "-------panBy----===left=" + mScrollRect.left);
		return mScrollRect.left * mScrollRect.left > 0;
	}
