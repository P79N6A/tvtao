...
/**
 * Created by chenjiajuan on 17/12/16.
 * 自适应LayoutManager
 */
...
public class FlowLayoutManager extends RecyclerView.LayoutManager {
    ...
    private SparseArray<Rect> allItemFrames = new SparseArray<>(); //保存所有的Item的上下左右的偏移量信息
    private HashMap<Integer,Integer> linesPosition=new HashMap<>();
    private SparseArray<View>  allViewArray=new SparseArray<>();   //保存所有的view
    ...
    public static class Row {
        ...
        float cuTop;                          //每一行的头部坐标
        float maxHeight;                      //每一行需要占据的最大高度
        List<Item> views = new ArrayList<>();  //每一行存储的item
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        for (int i = 0; i < getItemCount(); i++) {
            AppDebug.d(TAG, "index:" + i);
            View childAt = recycler.getViewForPosition(i);
             allViewArray.put(i,childAt);
            ...
            if (cuLineWidth + childUseWidth <= usedMaxWidth) {
                ...
                linesPosition.put(i,linesIndex);
            } else {
                //换行
                formatAboveRow();
                linesPosition.put(i,linesIndex);
    ...
    public View getFirstView(){
        for (int i=0;i<allViewArray.size();i++){
            View view=allViewArray.get(i);
    ...
    public int getLinesIndex(int position){
        if (linesPosition.isEmpty()){
            return 0;
        }
        AppDebug.e(TAG,"position = "+position+", linesPosition = "+linesPosition.toString());
        return linesPosition.get(position);
    }
     ...
     * @return
     */
    public boolean nextLines(int position){
        int lineSize=lineRows.size();
        int count=0;
        int j=0;
        int k=0;
        ...
        for (int i=0;i<lineSize;i++){
            List<Item> views=lineRows.get(i).views;
            int viewSize=views.size();
            count+=viewSize;
            k++;
            if (k<lineSize){
                viewSizek=lineRows.get(k).views.size();
                viewSizek+=count;
            }
            if (positionIndex<=count){
                j=i;
                break;
            }else if (positionIndex>count&&positionIndex<=viewSizek){
                j=k;
                break;
            }
        }
        //判断是否在最后一行，若在最后一行，则不能继续往下，拦截向下的事件
        if (j==(lineSize-1)){
            return false;
