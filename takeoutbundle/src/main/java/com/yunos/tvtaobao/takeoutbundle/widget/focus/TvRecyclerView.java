package com.yunos.tvtaobao.takeoutbundle.widget.focus;
...
public class TvRecyclerView extends RecyclerView {
    ...
    public void addFocusables(ArrayList<View> views, int direction, int focusableMode) {
        AppDebug.i(TAG, "views = " + views);
        AppDebug.i(TAG, "lastFocusView = " + mLastFocusView + " mLastFocusPosition = " + mLastFocusPosition);
        if (this.hasFocus() || mLastFocusView == null) {
            ...
            AppDebug.i(TAG, "views.add(lastFocusView)");
        }
    }
    ...
    public View focusSearch(View focused, int direction) {
        ...
        AppDebug.i(TAG, "focused = " + focused);
        AppDebug.i(TAG, "nextFocus = " + nextFocus);
        AppDebug.i(TAG, "realNextFocus = " + realNextFocus);
        //canScrollVertically(1)  true表示能滚动，false表示已经滚动到底部
        //canScrollVertically(-1) true表示能滚动，false表示已经滚动到顶部
        AppDebug.i(TAG, "canScrollVertically(-1) = " + canScrollVertically(-1));
        AppDebug.i(TAG, "canScrollVertically(1) = " + canScrollVertically(1));
        AppDebug.i(TAG, "canScrollHorizontally(-1) = " + canScrollHorizontally(-1));
        AppDebug.i(TAG, "canScrollHorizontally(1) = " + canScrollHorizontally(1));
        switch (direction) {
            case FOCUS_RIGHT:
                //调用移出的监听
    ...
    public void requestChildFocus(View child, View focused) {
        if (null != child) {
            AppDebug.i(TAG, "nextchild = " + child + ",focused = " + focused);
            if (!hasFocus()) {
            ...
            AppDebug.i(TAG, "focusPos = " + mLastFocusPosition);
        ...
        AppDebug.i(TAG, "mSelectedItemOffsetStart = " + mSelectedItemOffsetStart);
        AppDebug.i(TAG, "mSelectedItemOffsetEnd = " + mSelectedItemOffsetEnd);
    }
    ...
    public boolean requestChildRectangleOnScreen(View child, Rect rect, boolean immediate) {
        ...
        if (dx != 0 || dy != 0) {
            AppDebug.i(TAG, "dx = " + dx);
            AppDebug.i(TAG, "dy = " + dy);
            if (immediate) {
                scrollBy(dx, dy);
            } else {
