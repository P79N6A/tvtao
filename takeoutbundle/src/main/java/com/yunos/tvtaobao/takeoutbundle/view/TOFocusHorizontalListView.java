...
public class TOFocusHorizontalListView extends HListView implements DeepListener, ItemListener {

    private final String TAG = "TOFocusHorizonListView";
    ...
    public TOFocusHorizontalListView(Context context) {
        super(context);
    }

    public TOFocusHorizontalListView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public TOFocusHorizontalListView(Context context, AttributeSet attrs, int defStyle) {
        ...
        try {
            ...
            if (sel != null) {
                ...
                if (mTouchMode > TOUCH_MODE_DOWN && mTouchMode < TOUCH_MODE_SCROLL) {
                    ...
                    if (child != null)
                        positionSelector(mSelectedPosition, child);
                } else {
                    // mSelectedTop = 0;
                    mSelectorRect.setEmpty();
                }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        super.superOnMeasure(widthMeasureSpec, heightMeasureSpec);

        int index = getChildCount() > 0 ? getChildCount()/2: -1;

        if(index >=0){
            mItemWidth = getChildAt(index).getMeasuredWidth();
        }
...
//        }
    ...
    int amountToCenterScroll(int direction, int nextSelectedPosition) {
        mMotionPosition = nextSelectedPosition;
    ...
    private int getItemDistance(int startPos, int endPos, int direction) {
        BaseAdapter groupAdapter = (BaseAdapter)getAdapter();
        // 有效性
        ...
        int itemWidth = getResources().getDimensionPixelSize(R.dimen.to_ol_shop_cart_item_card_width);
        int totalWidth = 0;
        if (direction == View.FOCUS_DOWN) {
            if (startPos == Integer.MAX_VALUE) {
                totalWidth += (endPos + 1) * itemWidth;
            } else {
                totalWidth += (endPos - startPos) * itemWidth;
            }
        } else if (direction == View.FOCUS_UP) {
            if (startPos == Integer.MAX_VALUE) {
                totalWidth += endPos * itemWidth;
            } else {
                totalWidth += (endPos - startPos) * itemWidth;
            }
        }
