...
public class FocusOriginalPositionManager extends FrameLayout implements PositionListener {
    ...
    public FocusOriginalPositionManager(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init();
    }

    public FocusOriginalPositionManager(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public FocusOriginalPositionManager(Context context) {
        super(context);
        init();
    }
    ...
    private void drawFocus(Canvas canvas) {
        if (DEBUG) {
            Log.i(TAG, TAG + ".drawFocus.mFocused = " + mFocused + ".layoutDone = " + layoutDone);
        }
        if (mFocused != null && layoutDone) {
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        if (checkValidKey(event.getKeyCode()) &&
                (mFocused == null || !mFocused.hasFocus()) &&
                layoutDone) {
            Log.w(TAG,
                    "dispatchKeyEvent mFocused is null, may be no focusable view in your layout, or mouse switch to key");
            deliverFocus();
            invalidate();
            return true;
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (hasFocus() && !layoutDone && mFocused == null) {
            deliverFocus();
        }
        layoutDone = true;
    }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        Log.i(TAG, "onKeyDown the new focused = " + focused);
    ...
    public void reset() {
        ...
        if (mFocused != null) {
            ...
            if (mFocused != null) {
                //TODO ORIGINAL
                //mFocused.setPivotX(mFocused.getWidth() / 2);
                //mFocused.setPivotY(0);
                mFocused.requestFocus();
                mPositionManager.reset((FocusListener) mFocused);
            } else {
                isInvalidate = false;
