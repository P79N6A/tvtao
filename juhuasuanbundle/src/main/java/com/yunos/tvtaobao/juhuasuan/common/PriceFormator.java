...
public class PriceFormator {
     ...
     * 格式化为头部显示模式
     * 
     * @date 2012-11-1上午11:58:50
     * @param price
     * @return
     */
    public static SpannableString formatLongPrice(Long price) {
    ...
    public static SpannableString formatDoublePrice(double price, boolean withSymbol) {
        StringBuilder sb = new StringBuilder(String.valueOf(price));
        int index = sb.lastIndexOf(".");
        if (index != -1) {
            for (int i = 1, len = 2 - (sb.length() - index - 1); i <= len; i++) {
                sb.append("0");
            }
        }
        if (withSymbol) {
    ...
    public static String formatSmallTranAndQuantity(int quantity, long transportCharges) {
        ...
        transportChargesStr = transportCharges > 0 ? "(含快递 " + PriceFormator.formatNoSymbolLong(transportCharges) + " 元) " : "";
