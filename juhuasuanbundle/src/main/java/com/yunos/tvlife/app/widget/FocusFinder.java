...
public class FocusFinder {
    ...
    ArrayList<View> mFocusables = new ArrayList<View>(24);
    ...
    public View findNextFocus(ViewGroup root, View focused, int direction) {
        ...
        if (focused != null) {
            ...
            Log.i("Focusfinder", "Focusfinder.findNextFocus 1==>mFocusedRect=" + mFocusedRect);
            root.offsetDescendantRectToMyCoords(focused, mFocusedRect);
            Log.i("Focusfinder", "Focusfinder.findNextFocus 2==>mFocusedRect=" + mFocusedRect);
        } else {
    ...
    private View findUserSetNextFocus(View root, int direction, View v) {
        switch (direction) {
            case View.FOCUS_LEFT:
            case View.FOCUS_FORWARD:
                if (v.getNextFocusLeftId() == View.NO_ID)
                    return null;
            ...
            case View.FOCUS_UP:
                if (v.getNextFocusUpId() == View.NO_ID)
                    return null;
                return root.findViewById(v.getNextFocusUpId());
            case View.FOCUS_DOWN:
                if (v.getNextFocusDownId() == View.NO_ID)
                    return null;
    ...
    private View findNextFocus(ViewGroup root, View focused, Rect focusedRect, int direction) {
        ArrayList<View> focusables = mFocusables;
        if (focusables.isEmpty()) {
            // The focus cannot change.
            return null;
        }

        if (mSequentialFocusComparator.getRoot() == null) {
            mSequentialFocusComparator.setRoot((ViewGroup) root.getRootView());
        }
        ...
        for (int i = 0; i < numFocusables; i++) {
            ...
            //			mSequentialFocusComparator.getRect(focusable, mOtherRect);
            //            Rect r = getRect(focusable);
            //            mOtherRect.set(r);
            //            Log.i("aabb", "--focus---mOtherRect:" + mOtherRect.toShortString());
            ...
            if (isBetterCandidate(direction, focusedRect, mOtherRect, mBestCandidateRect)) {
                ...
                closest = focusable;
                Log.i("Focusfinder", "Focusfinder.findNextFocus end==>closest=" + closest);
            }
        }
        return closest;
    }
     ...
     */
    protected boolean isBetterCandidate(int direction, Rect source, Rect rect1, Rect rect2) {
     ...
     *         being in src's beam
     */
    protected boolean beamBeats(int direction, Rect source, Rect rect1, Rect rect2) {
    ...
    int getWeightedDistanceFor(int majorAxisDistance, int minorAxisDistance) {
        float floatAxisDistance = (float) majorAxisDistance / 100;
        float floatMinorAxisDistance = (float) minorAxisDistance / 100;

        return (int) (13 * floatAxisDistance * floatAxisDistance + floatMinorAxisDistance * floatMinorAxisDistance);
    }
     ...
     */
    protected boolean isCandidate(Rect srcRect, Rect destRect, int direction) {
        int diff = 1;
        switch (direction) {
            case View.FOCUS_LEFT:
            case View.FOCUS_FORWARD:
                return (srcRect.right > destRect.right || srcRect.left >= destRect.right)
                        ...
                        && (srcRect.top <= destRect.top && srcRect.bottom >= destRect.top
                                || srcRect.top <= destRect.bottom && srcRect.bottom >= destRect.bottom || srcRect.top >= destRect.top
            ...
            case View.FOCUS_UP:
                return (srcRect.bottom > destRect.bottom || srcRect.top >= destRect.bottom)
                        ...
                        && (srcRect.left <= destRect.left && srcRect.right >= destRect.left
                                || srcRect.left <= destRect.right && srcRect.right >= destRect.right || srcRect.left >= destRect.left
                                && srcRect.right <= destRect.right);
            case View.FOCUS_DOWN:
                Log.i("Focusfinder", "Focusfinder.isCandidate 3==>srcRect=" + srcRect + ", destRect=" + destRect);
    ...
    /**
     * Do the "beams" w.r.t the given direcition's axis of rect1 and rect2
     * overlap?
     * @param direction
     *            the direction (up, down, left, right)
     * @param rect1
     ...
     *         direction from source, return 1 (to break ties with {@link #majorAxisDistance}).
     */
    ...
    /**
     * Sorts views according to their visual layout and geometry for default tab
     * order. This is used for sequential focus traversal.
     */
    private static final class SequentialFocusComparator implements Comparator<View> {
        ...
        public int compare(View first, View second) {
            ...
            if (mFirstRect.top < mSecondRect.top) {
                ...
                // means that the views will retain their layout order relative
                // to one another.
                return 0;
            }
        }
