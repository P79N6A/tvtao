...
//		if (Math.abs(currentSelectedPosition) == 1) {
...
//		}
	...
	public void autoSelection(int position) {
		int currentSelectedPosition = position - mSelectedPosition;
		if (Math.abs(currentSelectedPosition) == 1) {
			if (currentSelectedPosition == 1) {
				arrowScrollImpl(View.FOCUS_RIGHT);
...
//		requestLayout();
	...
	View addViewFoward(View theView, int position, int count) {
		...
		for (int i = 0; i < count; i++) {
			nextView = addViewFoward(nextView, belowPosition);
			// belowPosition++;
			belowPosition--;
		}
		return nextView;
	...
	View addViewBackward(View theView, int position, int count) {
		...
		View nextView = theView;
		for (int i = 0; i < count; i++) {
			nextView = addViewBackward(nextView, belowPosition);
			// belowPosition--;
			belowPosition++;
		}
		return nextView;
	...
	private View fillSpecific(int position, int Left) {
		...
		if (!mStackFromRight) {
			above = fillRight(position - 1, temp.getLeft() - dividerWidth);
			// This will correct for the top of the first view not touching the
			// top of the list
			adjustViewsRightOrLeft();
			below = fillLeft(position + 1, temp.getRight() + dividerWidth);
			int childCount = getChildCount();
			if (childCount > 0) {
	...
	private void fillToGalleryLeftRtl() {
		...
		while (curRightEdge > galleryLeft && curPosition < mItemCount) {
			prevIterationView = makeAndAddView(curPosition, curRightEdge, false, mSpinnerPadding.top, mSelectedPosition == curPosition);
	...
	private void fillToGalleryRightRtl() {
		...
		while (curLeftEdge < galleryRight && curPosition >= 0) {
			prevIterationView = makeAndAddView(curPosition, curLeftEdge, true, mSpinnerPadding.top, mSelectedPosition == curPosition);
	...
	private View fillRight(int pos, int nextLeft) {
		...
		while (nextLeft < end && pos < mItemCount) {
			...
			// if(DEBUG)Log.d(TAG, "fillRight mSelectedPosition = " +
			// mSelectedPosition + ", mItemWidth = " + mItemWidth
			// + ", mItemHeight = " + mItemHeight + ", end = " + end +
			// ", nextLeft = " + nextLeft);
			pos++;
		}
		//
