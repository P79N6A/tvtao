...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
    ...
    public void run() {
        ...
        // 取消加载，虽然[delayIfNeed]这个函数中也可以取消加载，
        // 但两者会有不同点：
        //  delayIfNeed: 针对同一个View 当前显示的图片与之前不同
        //  cancelLoadIfneed： 可以针对不同的 view 来取消加载图片
        // @yunzhong.qyz
        if (cancelLoadIfneed())
            return;
    ...
    private Bitmap tryLoadBitmap() throws TaskCancelledException {
        ...
        try {
            ...
            //从硬盘获取
            if (imageFile.exists() && options.isCacheOnDisc()) {
                //  从Disc中取， 一般如果设置为缓冲到Disc中的，那也认为也从缓冲取
            ...
            if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
                ...
                // 如果需要从CacheOnDisc中加载
                String imageUriForDecoding = options.isCacheOnDisc() && tryCacheImageOnDisc(imageFile) ? cacheFileUri
                        : uri;
    ...
    private Bitmap decodeImage(String imageUri) throws IOException {
        ...
        if (imageLoadingInfo != null) {
            bitmapName = imageLoadingInfo.bitmapname;
        }
        ImageDecodingInfo decodingInfo = new ImageDecodingInfo(memoryCacheKey, imageUri, bitmapName, targetSize,
                engine, viewScaleType, getDownloader(), options);
    ...
    private boolean resizeAndSaveImage(File targetFile, int maxWidth, int maxHeight) throws IOException {
        ...
        ImageDecodingInfo decodingInfo = new ImageDecodingInfo(memoryCacheKey, Scheme.FILE.wrap(targetFile
                .getAbsolutePath()), bitmapName, targetImageSize, engine, ViewScaleType.FIT_INSIDE, getDownloader(),
                specialOptions);
            ...
            public void run() {
               try {
                   if (options.shouldShowImageOnFail()) {
                       imageAware.setImageDrawable(options.getImageOnFail(configuration.context.get().getResources()));
                   }
    ...
    private boolean cancelLoadIfneed() {
        boolean result = !engine.loadIfNeed(memoryCacheKey);
        return result;
    }
     ...
     *         doesn't match to image URI which is actual for current ImageAware at this moment)); <b>false</b> -
     *         otherwise
     */
