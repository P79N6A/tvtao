...
public class ImageLoaderEngine {
    ...
    // 此线程池作为临时缓冲线程池使用，不能删掉
    private Executor taskDistributor;

    private final Map<String, ImageAware> loadingUrl = Collections.synchronizedMap(new HashMap<String, ImageAware>());
    ...
    ImageLoaderEngine(ImageLoaderConfiguration configuration) {
        ...
        taskExecutor = null;
        taskExecutor = createTaskExecutor("Url");

        //        taskExecutor = configuration.taskExecutor; 
        //        taskExecutorForCachedImages = configuration.taskExecutorForCachedImages; 

        taskDistributor = creatTaskDistributor();
    ...
    //    /** Submits task to execution pool */
    //    void submit(final LoadAndDisplayImageTask task) throws RejectedExecutionException {  
    //        if (taskExecutor != null) { 
    //            taskExecutor.execute(task); 
    //        }
    //    }
    ...
    void submit(final LoadAndDisplayImageTask task) throws RejectedExecutionException {
        ...
        if ((taskDistributor == null) || (((ExecutorService) taskDistributor).isShutdown())) {
    ...
    private void initExecutorsIfNeed() {
        // if (!configuration.customExecutor && ((ExecutorService)
        // taskExecutor).isShutdown()) {
        // taskExecutor = createTaskExecutor("Url");
        // }
        // if (!configuration.customExecutorForCachedImages &&
        // ((ExecutorService) taskExecutorForCachedImages).isShutdown()) {
        // taskExecutorForCachedImages = createTaskExecutor("Cache");
        // }

        if ((taskExecutor == null) || (((ExecutorService) taskExecutor).isShutdown())) {
            taskExecutor = null;
            taskExecutor = createTaskExecutor("Url");
        }
    }
    ...
    private Executor createTaskExecutor(String name) {
        return DefaultConfigurationFactory.createExecutor(name, configuration.threadPoolSize,
                configuration.threadPriority, configuration.tasksProcessingType);
    }

    private Executor creatTaskDistributor() {
        return Executors.newCachedThreadPool(new DefaultConfigurationFactory.CachedThreadPoolThreadFactory(
                "TaskDistributor"));
    }
     ...
     * define image URI is loaded into View at
     * exact moment.
     */
    void prepareDisplayTaskFor(ImageAware imageAware, String memoryCacheKey) {
        ...
        // 保存URL地址
        loadingUrl.put(memoryCacheKey, imageAware);
    }
     ...
     */
    void cancelLoadTaskFor(String memoryCacheKey) {
        loadingUrl.remove(memoryCacheKey);
    }

    /**
     * 取消全部的加载
     */
    void cancelLoadAllTaskFor() {
        loadingUrl.clear();
        cacheKeysForImageAwares.clear(); 
        
        // 清除线程池
        L.i("taskExecutor = " + taskExecutor + "; taskDistributor = " + taskDistributor);
        
        if (taskExecutor != null && taskExecutor instanceof ThreadPoolExecutor) {
            ((ThreadPoolExecutor) taskExecutor).purge();
        } 
        if (taskDistributor != null && taskDistributor instanceof ThreadPoolExecutor) {
            ((ThreadPoolExecutor) taskDistributor).purge();
        }
    }
    ...
    public boolean loadIfNeed(String memoryCacheKey) {
        return loadingUrl.containsKey(memoryCacheKey);
    }
     ...
     * <b>imageAware</b>.
     * @param imageAware
     *            {@link com.nostra13.universalimageloader.core.imageaware.ImageAware} for which display task
     *            will be cancelled
     */
    ...
    /**
     * Denies or allows engine to download images from the network.<br /> <br />
